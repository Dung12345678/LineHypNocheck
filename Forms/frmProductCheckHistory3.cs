...
namespace BMS
{
    public partial class frmProductCheckHistory3 : _Forms
    {
        ...
        private void frmProductCheckHistory1_Load(object sender, EventArgs e)
        {
            ...
            //andonActive = TextUtils.ToInt(TextUtils.ExcuteScalar($"select top 1 KeyValue from ConfigSystem where KeyValue = 'IsAndonActiveHyp'"));
            ...
            while (true)
            {
                ...
                try
                {
                    ...
                    for (int i = 1; i <= 14; i++)
                    {
                        ...
                        {
                            Control control = pannelBot.Controls.Find("lbl" + step, true)[0];
                            switch (_status)
                            {
                                case 1:
        ...
        void checkFreeze()
        {
            while (true)
            {
                ...
                try
                {
                    ...
                    /*if (currentStatus == 1 || currentStatus == 2 || currentStatus == 5 && enable == false)
					{
						enable = true;
					}
					else
        ...
        void loadGridData()
        {
            ...
            {
                string sql = $"select top 1 * from ConfigFormula where ProductTypeCode = '{productTypeCode}' and ProductGroupCode = '{productGroupCode}'";
                DataTable dt = TextUtils.Select(sql);
                if (dt == null) return;
                if (dt.Rows.Count == 0) return;
                _formulaX1 = TextUtils.ToString(dt.Rows[0]["Formula1"]);
                _formulaX2 = TextUtils.ToString(dt.Rows[0]["Formula2"]);
                if (_formulaX1 != "")
                {
                    int sortOrderX1 = TextUtils.ToInt(_formulaX1.Split(';')[1]);
                    _formulaX1 = _formulaX1.Split(';')[0];
                    _sortOrderMin = sortOrderX1;// < sortOrderX2 ? sortOrderX1 : sortOrderX2;
                }
                if (_formulaX2 != "")
                {
                    int sortOrderX2 = TextUtils.ToInt(_formulaX2.Split(';')[1]);
                    _formulaX2 = _formulaX2.Split(';')[0];
                    _sortOrderMax = sortOrderX2;// < sortOrderX2 ? sortOrderX2 : sortOrderX1;
                }
                if (_formulaX1 == "" || _formulaX2 == "")
                {
                    _value = _sortOrderMin < _sortOrderMax ? _sortOrderMax : _sortOrderMin;
                }
                else
                    _value = _sortOrderMin < _sortOrderMax ? _sortOrderMin : _sortOrderMax;
                //}
                for (int i = 0; i < grvData.RowCount; i++)
                {
                ...
                for (int i = 0; i < grvData.RowCount; i++)
                {
                    ...
                    if (sortOrder < _sortOrderGMD && sortOrder < _value)
                    {
                        _sortOrderBehind = sortOrder;
                        _sortOrderCalculate = sortOrder;
                    }
                    if (sortOrder > _sortOrderGMD && sortOrder < _value) _sortOrderCalculate = sortOrder;
                }
            }
        ...
        int _sortOrderMin = -1000;
        int _sortOrderMax = -1000;
        int _value = 0;
        ...
        void calculateShim(int columnindex, int rowIndex)
        {
            string fomulaX2 = _formulaX2;
            string fomulaX1 = _formulaX1;
            int rowIndexMin = 0;
            int rowIndexMax = 0;
            for (int i = 0; i < grvData.RowCount; i++)
            {
                int sortOrder = TextUtils.ToInt(grvData.GetRowCellValue(i, colSortOrder));
                decimal value = TextUtils.ToDecimal(grvData.GetRowCellValue(i, "RealValue" + columnindex));

                if (sortOrder == _sortOrderMin)
                {
                    rowIndexMin = i;
                }
                if (sortOrder == _sortOrderMax)
                {
                    rowIndexMax = i;
                }

                fomulaX2 = fomulaX2.Replace($"#{sortOrder}#", value.ToString());
                fomulaX1 = fomulaX1.Replace($"#{sortOrder}#", value.ToString());
            }
            if (fomulaX1 != "")
            {
                decimal valueMin = Math.Truncate((decimal)sc.Eval(fomulaX1));
                grvData.SetRowCellValue(rowIndexMin, "RealValue" + (columnindex), valueMin);
            }
            if (fomulaX2 != "")
            {
                decimal valueMax = Math.Truncate((decimal)sc.Eval(fomulaX2));
                grvData.SetRowCellValue(rowIndexMax, "RealValue" + (columnindex), valueMax);
            }

        }
        //void calculateX1(int columnindex, int rowIndex)
        //{
        //    string fomulaX2 = _formulaX2;
        //    string fomulaX1 = _formulaX1;
        //    int rowIndexMin = 0;
        ...
        //    {
        //        int sortOrder = TextUtils.ToInt(grvData.GetRowCellValue(i, colSortOrder));
        //        decimal value = TextUtils.ToInt(grvData.GetRowCellValue(i, "RealValue" + columnindex));
        ...
        //    decimal valueMin = (decimal)sc.Eval(fomulaX1);
        //    //decimal valueMax = (decimal)sc.Eval(fomulaX2);

        //    //if (rowIndexMax != rowIndex) grvData.SetRowCellValue(rowIndexMax, "RealValue" + (columnindex), valueMax);
        //    if (rowIndexMin != rowIndex) grvData.SetRowCellValue(rowIndexMin, "RealValue" + (columnindex), valueMin);
        //}
        private void grvData_CellValueChanged(object sender, DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs e)
        {
            ...
            if (e.RowHandle >= 0 && e.Column.VisibleIndex > 2)
            {
                ...
                if (NoIndefinitely != e.RowHandle)
                {
                    ...
                    calculateShim(index, e.RowHandle);
                }
                if (sortOrder == _sortOrderCalculate)
                {
                    (pannelBot.Controls.Find("textBox" + index, true)[0]).Focus();
                }
                else
                {
                    if (sortOrder == _sortOrderBehind)
                    {
                        this.setFocusCell(getNextIndex(e.RowHandle + 1), grvData.FocusedColumn.VisibleIndex);
                    }
                    else
                        this.setFocusCell(getNextIndex(e.RowHandle), grvData.FocusedColumn.VisibleIndex);
                }
        ...
        private void textBox_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (e.KeyCode == Keys.Enter)
            {
                ...
                if (_oAndonModel.ID > 0)
                {
                    ...
                    if (andonDetail.MakeTime > andonDetail.Takt)
                    {
                        andonDetail.Type = 1;
                        andonDetail.PeriodTime = andonDetail.MakeTime - _oAndonModel.Takt;
                    }
                    else
                    {
                        andonDetail.Type = 3;
